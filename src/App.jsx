...
function App() {
  useEffect(() => {
    ...
    // set up red box mesh
    const bg1 = new THREE.SphereGeometry(0.5);
    const bm1 = new THREE.MeshPhongMaterial({ color: 0xfafafa });
    const boxMesh1 = new THREE.Mesh(bg1, bm1);
    boxMesh1.castShadow = true;
    ...
    const bm2 = new THREE.MeshPhongMaterial({ color: 0xfafafa });
    ...
    const bg3 = new THREE.ConeGeometry(0.5);
    const bm3 = new THREE.MeshPhongMaterial({ color: 0xfafafa });
    ...
    // hemi    
    // set up hemisphere light + helper
    const skyColor = 0x0000FF;
    const groundColor = 0xB97A20;
    const intensity = 0.5;

    const hl = new THREE.HemisphereLight(skyColor, groundColor, intensity);
    hl.position.set(0, 2, 0);
    const hlHelper = new THREE.HemisphereLightHelper(hl);
    mainGroup.add(hl, hlHelper);
    ...
    hlFolder.add(hl, 'intensity', 0, 1, 0.2);
    hlFolder.open();
